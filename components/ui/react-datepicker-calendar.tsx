...
function ReactDatePickerCalendar({ selected, onSelect, className, mode = 'single', disabled, ...props }: ReactDatePickerCalendarProps) {
...
export function ReactDatePickerWrapper({ selected, onSelect, className, disabled, ...props }: ReactDatePickerCalendarProps) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant='outline' className={cn('w-full justify-start text-left font-normal', !selected && 'text-muted-foreground', className)}>
          <CalendarIcon className='mr-2 h-4 w-4' />
          {selected ? format(selected, 'PPP') : <span>Select date</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent className='w-auto p-0' align='start'>
        <ReactDatePickerCalendar selected={selected} onSelect={onSelect} disabled={disabled} initialFocus={props.initialFocus} />
      </PopoverContent>
    </Popover>
  );
}
