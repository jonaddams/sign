  ...
  const renderMobileTemplateCard = (template: Template) => {
    ...
    return (
      <Card
        ...
        onClick={() =>
          handleSelectTemplate(template.id, template.templateFilePath)
        }
      >
        <CardContent className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="line-clamp-1 font-medium">{template.name}</div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2 text-sm">
          Choose an existing template or upload a new document to send for
          signing.
        </p>
      </div>
      ...
      <Tabs defaultValue="upload" className="w-full">
        <TabsList
          className={
            isMobile
              ? "flex h-auto w-full flex-col space-y-2 py-2"
              : "mb-4 grid grid-cols-2"
          }
        >
          <TabsTrigger
            value="upload"
            className={`${isMobile ? "mb-2 w-full" : ""} cursor-pointer`}
          >
            Upload New Document
          </TabsTrigger>
          <TabsTrigger
            value="template"
            className={`${isMobile ? "w-full" : ""} cursor-pointer`}
          >
            Use Template
          </TabsTrigger>
        </TabsList>
                ...
                {isMobile ? (
                  <div className="space-y-2">
                    {currentTemplates.map((template) =>
                      renderMobileTemplateCard(template),
                    )}
                  </div>
                ) : (
