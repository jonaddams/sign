...
export default function Page() {
  ...
  function getSelectedUserName(uuid) {
    let selectedUser = signers.find((signee) => signee.uuid === uuid);
    return selectedUser.name;
  }

  function handleSelect(event) {
    if (signers.length > 1) {
      let i = signers.findIndex((signee) => signee.uuid === event.target.value);
        ...
        <div className='flex'>
          <div className='w-full lg:w-1/4'>
            ...
            <div className='w-full border-b border-white bg-anti-flash-white flex justify-between content-center px-6 py-3'>
              <select className='select bg-anti-flash-white select-ghost w-full' onChange={handleSelect} ref={selectRef}>
                {signers.map((signee, index) => (
                  <option value={signee.uuid} key={signee.uuid} className='text-base normal-case leading-55 tracking-02 text-dark-gunmetal'>
                    {signee.name}
                  </option>
                ))}
              </select>
            ...
            <div className='py-5 border-anti-flash-white px-5'>
              <p className='text-smaller leading-4-half text-auto-metal-saurus tracking-005 mb-5'>Drag to add signature to the document.</p>
              <div
